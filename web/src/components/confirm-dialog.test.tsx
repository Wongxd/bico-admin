...
describe('ConfirmDialog', () => {
  ...
  it('does not close from overlay while loading', async () => {
    const onOpenChange = vi.fn()
    await render(
      <ConfirmDialog
        open
  ...
  it('renders confirm as submit button linked to desc form when `form` is set', async () => {
    const { getByRole } = await render(
      <ConfirmDialog
        ...
        form='records-multi-delete-form'
        desc={
          <form id='records-multi-delete-form' className='space-y-4'>
    ...
    await expect
      .element(deleteBtn)
      .toHaveAttribute('form', 'records-multi-delete-form')
  })
